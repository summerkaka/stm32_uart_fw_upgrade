...
static int8_t answer(uint8_t num, ...)
{
    va_list valist;
    int8_t i = 0, idx = 0;
    dtype_t dtype = k_u8;
    ...
    for (; i < num; i++) {
        if (i % 2 == 0) {
            ...
            switch (dtype) {
            case k_u8:
                cmd_answer[idx++] = va_arg(valist, int);
                break;
            case k_u16:
                data = va_arg(valist, int);
                WriteShortH(cmd_answer+idx, data);
                idx += 2;
                break;
            case k_u32:
                data = va_arg(valist, int);
                WriteWordH(cmd_answer+idx, data);
                idx += 4;
                break;
            case k_u64:
                data = va_arg(valist, uint64_t);
                WriteDWordH(cmd_answer+idx, data);
                idx += 8;
                break;
            case k_f32:
                data32.d_float = va_arg(valist, double);
                WriteWordH(cmd_answer+idx, data32.d_uint32);
                idx += 4;
                break;
            case k_f64:
                data64.d_double = va_arg(valist, double);
                WriteDWordH(cmd_answer+idx, data64.d_uint64);
                idx += 8;
                break;
            default : break;
            }
    ...
    va_end(valist);
    WriteShortH(&cmd_answer[idx], 0x0000); // write crc
    idx += 2;
    WriteShortH(&cmd_answer[idx], 0x0d0a); // write frame-end-symbol
    idx += 2;
    // for (i=0; i<idx+2; i++) {
    //     if (HAL_UART_Transmit(UART, &cmd_transmit[i], 1, 0xffff) != HAL_OK)
    //         return -1;
    // }
    i= HAL_UART_Transmit(UART, cmd_transmit, idx+2, 0xffff);
    // memset(cmd_answer, 0, idx);
    return i;
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    case kConsoleRecv:
        ...
        if (widx > IDX_LEN && widx >= cmd_recv[IDX_LEN] + 2 + 4) {
...
{
    uint8_t i = 0, ret = HAL_ERROR, cmd_rsp;
    uint8_t length;
    ...
    if (uart_sts == kConsoleCplt) {
        ...
        cmd_rsp = cmd_exec[IDX_CMD] | 0x80;

        switch (cmd_exec[IDX_CMD]) {
        case CMD_UPDATE_PING:
            answer(6, k_u8, cmd_rsp, k_u8, 0x01, k_u8, 0xa5);
            break;
        case CMD_ASK_APPAREA:
            answer(8, k_u8, cmd_rsp, k_u8, 0x08, k_u32, FLASH_USER_START_ADDR, k_u32, FLASH_USER_END_ADDR);
            break;
        case CMD_PROGRAM_START:
            if ((ret = HAL_FLASH_Unlock()) == HAL_OK) {
                erase_init.TypeErase   = FLASH_TYPEERASE_PAGES;
                erase_init.PageAddress = FLASH_USER_START_ADDR;
                erase_init.NbPages     = (FLASH_USER_END_ADDR - FLASH_USER_START_ADDR) / FLASH_PAGE_SIZE + 1;
        ...
        case CMD_JUMPTOAPP:
            if (((*(__IO uint32_t *)APP_ADDRESS) & 0x2FFE0000) == 0x20000000) {
                answer(6, k_u8, cmd_rsp, k_u8, 0x01, k_u8, 0);
